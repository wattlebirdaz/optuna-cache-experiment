...
if __name__ == "__main__":
    ...
    if args.backend == "mysql":
        storage = RDBStorage("mysql+mysqldb://optuna:password@127.0.0.1:3306/optuna")
        cached_storage = _CachedStorage(
            RDBStorage("mysql+mysqldb://optuna:password@127.0.0.1:3307/optuna")
        )
    elif args.backend == "sqlite3":
        storage = RDBStorage("sqlite:///db.sqlite3withoutcache")
        cached_storage = _CachedStorage(RDBStorage("sqlite:///db.sqlite3withcache"))
    elif args.backend == "postgres":
        storage = RDBStorage("postgresql+psycopg2://root:root@localhost:5432/optuna")
        cached_storage = _CachedStorage(
            RDBStorage("postgresql+psycopg2://root:root@localhost:5433/optuna")
        )
    else:
        raise RuntimeError
